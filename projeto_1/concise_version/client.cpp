...
int main()
{
    ...
    ThreadArgs temp_args = {TEMP_PORT, "Temperatura"};
    ThreadArgs humid_args = {HUMID_PORT, "Umidade"};
    ThreadArgs press_args = {PRESS_PORT, "Pressão"};
...
void *receive_data(void *arg)
{
    ...
    int sockfd;
    {
        ...
        std::cout << "Abertura de conexão com o socket de |" << args->desc << "|\t" << " na porta \t|"<< args->port << "|"
                  << std::endl;
    }
    while (true)
    {
    ...
    std::cout << args->desc << " recebida: " << buffer << std::endl;
    close(sockfd);
    return NULL;
}
